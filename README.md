...
Мод пишет в `BepInEx/LogOutput.log` сообщения о загрузке, вызове `Update` и `OnGUI`, разрешении экрана и текущем значении голода. Если полоса не появилась, найдите строки с `Hunger Bar` и `OnGUI` в этом файле. В версии 1.0.1 полоса принудительно рисуется поверх остальных IMGUI-элементов.
